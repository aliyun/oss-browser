          ...
          ossSvs2.getFileInfo($scope.bucketInfo.region, $scope.bucketInfo.bucket, $scope.objectInfo.path).then(function(data){
            ...
            if(data.Restore){
              ...
              if(info['ongoing-request']=='true'){
                ...
                $scope.info.msg = '归档文件，已恢复，可读截止时间：'+ moment(new Date(info['expiry-date'])).format('YYYY-MM-DD HH:mm:ss');
                $scope.info.showContent=true;
                $scope.info.msgbg = 'success';
                if(fn)fn()
              }
            }
            else{
              if($scope.objectInfo.storageClass=='Archive'){
                 $scope.info.msg = '归档文件，需要恢复才能预览或下载';
                 $scope.info.msgbg = 'danger';
                 $scope.info.showContent=false;
